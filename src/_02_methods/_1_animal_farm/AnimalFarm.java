...
public class AnimalFarm {
	...
	void animals() {
		 ...
		 */	
		for(int chez=0;chez<5;chez+=1) {
			String animal = JOptionPane.showInputDialog("GIMME AN ANIMAL");
			/* 2. Make it so that the user can keep entering new animals. */

			if(animal.toLowerCase().contains("cow")) {
				moo();
			}
			else if(animal.toLowerCase().contains("duck")) {
				quack();
			}
			else if(animal.toLowerCase().contains("dog")) {
				woof();
			}
			else if(animal.toLowerCase().contains("cat")) {
				meow();
			}
			else if(animal.toLowerCase().contains("llama")) {
				llamaScream();
			}else {
				JOptionPane.showMessageDialog(null, "Nope! Goodbye");
				System.exit(0);
			}
		}
	JOptionPane.showMessageDialog(null, "Congrats, you made it! Have a good day:)");
	System.exit(0);
	}
